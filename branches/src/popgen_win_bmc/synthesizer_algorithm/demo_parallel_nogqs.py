...
import synthesizer_algorithm.heuristic_algorithm_nogqs
...
# Checking marginal totals
    ...
    person_marginals = synthesizer_algorithm.adjusting_sample_joint_distribution.prepare_control_marginals (db, 'person', person_control_variables,
                                                                                                            varCorrDict,
...
# Running IPF for Persons
    print 'Step 2B: Running IPF procedure for Persons... '
    person_objective_frequency, person_estimated_constraint = synthesizer_algorithm.ipf.ipf_config_run(db, 'person', person_control_variables, 
                                                                                 varCorrDict,
                                                                                 project.adjControlsDicts.person, 
                                                                                 person_dimensions, 
                                                                                 state, county, pumano, tract, bg, 
...
# Creating the control array
    total_constraint = numpy.hstack((hhld_estimated_constraint[:,0], person_estimated_constraint[:,0]))
...
# Running the heuristic algorithm for the required geography
	...
	print 'Employing the proportional updating procedure for reallocating sample weights', project.parameters.ipuProcedure
    	iteration, weights, conv_crit_array, wts_array = synthesizer_algorithm.heuristic_algorithm_nogqs.heuristic_adjustment(db, 0, index_matrix, weightsDef, total_constraint, sp_matrix, parameters)
	...
	print 'Employing the entropy-based updating procedure for reallocating sample weights', project.parameters.ipuProcedure
    	iteration, weights, conv_crit_array, wts_array = synthesizer_algorithm.heuristic_algorithm_nogqs.ipu_entropy(db, 0, index_matrix, weightsDef, total_constraint, sp_matrix, parameters)
...
# Sampling Households and choosing the draw with the best match with with the objective distribution
    ...
    while(p_value < parameters.synPopPTol and draw_count < parameters.synPopDraws):
        ...
        synthetic_housing_units = synthesizer_algorithm.drawing_households.drawing_housing_units_nogqs(db, frequencies, weights, index_matrix, sp_matrix, 0)
