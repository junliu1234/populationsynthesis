...
# Running the heuristic algorithm for the required geography
	...
	print 'Employing the proportional updating procedure for reallocating sample weights', project.parameters.ipuProcedure
    	iteration, weights, conv_crit_array, wts_array = heuristic_algorithm_nogqs.heuristic_adjustment(db, 0, index_matrix, weightsDef, total_constraint, sp_matrix, parameters)
	...
	print 'Employing the entropy-based updating procedure for reallocating sample weights', project.parameters.ipuProcedure
    	iteration, weights, conv_crit_array, wts_array = heuristic_algorithm_nogqs.ipu_entropy(db, 0, index_matrix, weightsDef, total_constraint, sp_matrix, parameters)
...
# Sampling Households and choosing the draw with the best match with with the objective distribution
    ...
    while(p_value < parameters.synPopPTol and draw_count < parameters.synPopDraws):
        ...
        #synthetic_housing_units = drawing_households.drawing_housing_units_nogqs(db, frequencies, weights, index_matrix, sp_matrix, 0, drawingProcedure=project.parameters.drawingProcedure)
        synthetic_housing_units = drawing_households.drawing_housing_units_nogqs(db, frequencies, weights, index_matrix, 
									   sp_matrix, 0, 
