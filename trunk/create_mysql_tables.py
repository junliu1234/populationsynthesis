...
def create_tables(db, project_name, path):
	...
	dbc.execute('''Create Table hhld_pums(pumano int, hhldpumsid int,
							     hhldid int, childpresence int,
							     hhldtype int, hhldsize int,
							     hhldinc int)''')
	dbc.execute('''load data local infile '%s/hhld_pums.dat' into table hhld_pums'''%(path))
	
	dbc.execute(''' Create Table person_pums(pumano int, hhldpumsid int,
								 hhldid int, personid int,
								 gender int, age int, 
								 race int, employment int)''')
	dbc.execute('''load data local infile '%s/person_pums.dat' into table person_pums''' %(path))

	dbc.execute('''Create Table hhld_marginals(county int, pumano int,
								  tract int, bg int,
								  hhldtotal int, childpresence1 int,
