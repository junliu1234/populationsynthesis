...
# Running the heuristic algorithm for the required geography
    iteration, weights, conv_crit_array, wts_array = heuristic_algorithm_nogqs.heuristic_adjustment(db, 0, index_matrix, weights, total_constraint, sp_matrix, parameters)
...
# Creating synthetic hhld, and person attribute tables
    ...
    for i in range(len(person_control_variables)):
        print '%s variable\'s marginal distribution sum is %d' %(person_control_variables[i], round((person_marginals[i])).sum())
